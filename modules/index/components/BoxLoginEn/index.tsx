...
const BoxLoginEn: FC = () => {
  const normalText = "Dedicated online programming learning platform";
  const normalTextList = normalText.split(" ");
  const highLightText = "for students";
      ...
      <Container size="xl">
        <div className="w-full max-w-[1200px] m-auto flex flex-col gap-[42px] md:gap-[62px] relative">
                  ...
                  if (index > 0) {
                    return (
                      <motion.div
                        ...
                        initial={{ y: -30, opacity: 0 }}
                        animate={{ y: 0, opacity: 1 }}
                        transition={{ type: "spring", bounce: 0.5, duration: 0.5, delay: 0.05 * (normalTextList.length + index) || 0 }}
                      >
                        <span>{text}</span>
                      </motion.div>
                    );
          ...
          <div className="w-full sm:w-[48%] sm:max-w-[560px] flex flex-col gap-4">
            <span className="font-medium text-sm leading-[22px] text-white">Our partner:</span>
