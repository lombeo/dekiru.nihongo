...
const Breadcrumbs = (props: BreadcrumbsProps) => {
  ...
  const items = data.map((item, index) => {
    const color = isWhite ? "!text-white" : "!text-[#8899A8]";
    const bold = "";
    ...
    return (
      <Link href={item.href} className={`${color} ${bold} text-[13px]  hover:underline`} key={index} onClick={item?.onClick} >
        {item.title}
      </Link>
    );
  });
  ...
  return (
    <nav className="w-full py-5">
      <BreadcrumbsCore
        ...
        separator={<ChevronRight color={isWhite ? "white" : "#8899A8"} size={15} />}
      >
        {items}
      </BreadcrumbsCore>
    </nav>
