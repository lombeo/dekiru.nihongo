...
import { SQLActivitySchema } from "validations/cms/sqlActivity.schemal";
...
const activityCodeSubType = ActivityCodeTypeEnum.SQL;

export const SQLForm = (props: any) => {
  ...
  const {
    ...
    resolver: yupResolver(SQLActivitySchema),
    defaultValues: {
      ...data,
      language: keyLocale,
      activityCodeSubType,
  ...
  useEffect(() => {
    reset({
      ...
      listInputs: data.listInputs,
      listTestCase: data.listTestCase,
    });
  }, [data]);
  ...
  const onClickSave = (rs: any) => {
    ...
    fdata.append("file", watch("file")[0]);
    if (actionType == "edit") {
  ...
  const onAddNewTestCase = () => {
    const newList = [...(watch("listTestCase") || []), { executeLimitTime: 0, input: "", output: "" }];
  ...
  return (
    <>
      <Visible
        rules={[
          FunctionBase.ruleOperation(data?.activity?.ownerId == parseInt(profile?.userId), isAdmin, "or"),
          actionType == "create",
        ]}
        ruleOperation="or"
      >
        ...
        <Text className="text-2xl filter">
          {actionType === "create" ? t(LocaleKeys["Create"]) : t(LocaleKeys["Edit"])} SQL
        </Text>
