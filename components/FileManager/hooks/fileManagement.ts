...
export const useFileManagementData = (FilterItem: {}): any => {
  ...
  const updateFiles = (file: any) => {
    ...
    if (index >= 0) {
      files[index] = file;
      if (files.length > 0) files.sort((a: any, b: any) => a.name.localeCompare(b.name));
      setFiles([...files]);
    } else {
      const orderedFiles = [...files, ...[file]];
      if (orderedFiles.length > 0) orderedFiles.sort((a: any, b: any) => a.name.localeCompare(b.name));
  ...
  useEffect(() => {
    ...
    if (removeId != 0 && oldId != folderId) {
      ...
      if (currentPageIndex == 0 && filesData) {
        return setFiles(filesData.filter((item: any) => currentRemoveId != item.id));
      } else {
        filesData = filesData.filter((item: any) => currentRemoveId != item.id);
      }
    }
    FileManagementService.getFiles(folderId, fileExt, currentPageIndex, pageSize, orderBy)
      .then((res: any) => {
        oldId = folderId;
        if (res.data) {
          const data = res.data;
