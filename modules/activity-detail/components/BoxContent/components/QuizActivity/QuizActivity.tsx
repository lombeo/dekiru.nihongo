...
const QuizActivity = (props: any) => {
  ...
  const [afterSubmitTest, setAfterSubmitTest] = useState<boolean>(false);
  ...
  //On New Submit
  const onNewSubmit = (executions: any) => {
    const model = {
      quizTestId: testId,
      executions: executions,
      finishNow: true,
    ...
    QuizService.submitTestMulti(model).then((response: any) => {
      ...
      if (response?.data?.success) {
        QuizService.getTestDetail({
          quizTestId: response?.data?.data?.id,
        }).then((data: any) => {
          if (data.data.data) {
            setDataTakeQuiz(data.data.data);
          }
        });
        const filter = {
          activityId: activityId,
    ...
    <div>
      ...
      {isShowQuiz && (
        <TakeQuizLayout
          ...
          afterSubmitTest={afterSubmitTest}
          setAfterSubmitTest={setAfterSubmitTest}
          onViewAnswers={onViewAnswers}
          viewAnswers={activityData?.viewAnswers}
          suggestion={activityData?.suggestion}
          numberOfTries={activityData?.numberOfTries}
          roundCount={userTestData?.roundCount}
