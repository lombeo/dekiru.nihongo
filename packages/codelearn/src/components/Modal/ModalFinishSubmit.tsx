...
/**
 * Display finish modal after submit code success
 * @param props FinishSubmitProps
 * @returns Modal confirm for next activity of stay in current activity
 */
const ModalFinishSubmit = (props: FinishSubmitProps) => {
  const { open, onClose, nextActivity } = props;
  ...
  const activityURL = `/learning/${permalink}?activityType=${type}&activityId=${id}`;
  return (
    <Modal opened={open} onClose={onClose} size="md">
      <div className="flex flex-col item-center justify-center text-center mb-5">
        ...
        <p className="text-lg">{t("You have just finished this lesson.")}</p>
      </div>
      <div className="flex justify-center mb-4">
        ...
        {nextActivity && (
          <Link href={activityURL}>
            <a onClick={onClose}>
              <Button type="submit" className="ml-4" title={t("Next lesson")}>
                {t("Next lesson")}
              </Button>
            </a>
          </Link>
          // <Button type="submit" className="ml-4" component="a" href={activityURL} title={t("Next lesson")}>
          //   {t("Next lesson")}
          // </Button>
        )}
      </div>
