...
const ModalCompleteActivity = (props: ModalCompleteActivityProps) => {
  ...
  useEffect(() => {
    const pubsub = PubSub.subscribe(ACTIVITY_SUB_CHANEL.MARK_COMPLETE_ACTIVITY, (chanel, { activityId }) => {
  ...
  const onNextActivity = (event) => {
    event.preventDefault();
    router.push(activityNextLink).then(() => setIsOpen(false));
  };
...
<g id="Course" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
  ...
  return (
    <Modal centered onClose={() => setIsOpen(false)} opened={isOpen}>
      <div>
        ...
        <div className="flex gap-4 lg:ml-2 mt-2 lg:mt-0 justify-center">
          <Button variant="outline" onClick={() => setIsOpen(false)}>
            {t("Stay on this page")}
          </Button>
          {activityNextLink ? (
            <Link href={activityNextLink} onClick={onNextActivity}>
              <Button className="font-medium">{t("Next lesson")}</Button>
            </Link>
          ) : null}
        </div>
      </div>
