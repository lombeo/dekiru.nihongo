...
import GradeAssignmentModal from "@src/modules/activities/components/ActivityPresents/GradeAssignmentModal";
import SubmitAssignmentModal from "@src/modules/activities/components/ActivityPresents/SubmitAssignmentModal";
...
const AssignmentActivity = (props: AssignmentActivityProps) => {
  ...
  const disabledSubmit =
    !isEmpty(submission?.point) || !isEmpty(submission?.comment) || data.activityData?.metadata?.startDate == null;
    ...
    <>
      ...
      <div className="px-5">
        ...
        <div className="bg-[#F1F3F5] px-4 py-2 rounded-md min-h-[50px] flex flex-col justify-center">
          {!data.activityData?.metadata?.files?.length && !data.activityData?.metadata?.fileName && <span>--</span>}
        ...
        {(isGradeAssignment || isManager) && (
          <>
            ...
            <Button onClick={handleDownload} loading={loadingDownload} disabled={selected.length <= 0} className="mt-4">
              {t("Download")}
            </Button>
