...
const GradeModal = (props: any) => {
  const { onClose, onSuccess, activityId, selected, data: dataScratch } = props;
  ...
  const submit = () => {
    handleSubmit(async (data) => {
      ...
      //   return;
      // }
      if (dataScratch?.activityData?.metadata?.maxScore && data.point > dataScratch?.activityData?.metadata?.maxScore) {
        Notify.error(t("The score cannot exceed the maximum score"));
        return;
      }
      setLoadingForm(true);
      const res = await LearnActivityService.gradeScratch({
        activityId,
        activityTitle: dataScratch?.title,
        sessionId: 0,
  ...
  return (
    <>
      <Modal title={t("Grade assignment")} size="xl" opened zIndex={300} onClose={onClose}>
        <div>
          <Text className="text-xl mt-6 font-semibold mb-3">
            {t("Grade (Max score: {{score}})", { score: dataScratch?.activityData?.metadata?.maxScore })}
          </Text>
