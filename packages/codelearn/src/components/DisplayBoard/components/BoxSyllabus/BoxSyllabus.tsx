...
const BoxSyllabus = (props: BoxSyllabusProps) => {
  const { permalink, activitiesCompleted, activeId } = props;

  const { t } = useTranslation();

  const { data } = useQuery({
    queryKey: ["schedule-course", permalink],
    queryFn: async () => {
    ...
    data?.courseScheduleList?.map((e) => ({
      ...
      subItems: e.sections?.map((e1) => ({
        ...
        subItems: e1.activities?.map((e2) => ({
          ...
          activityStatus: activitiesCompleted?.includes(e2.activityId)
  ...
  return (
    <div className="px-4 py-2">
      <div className="font-semibold">{t("Syllabus")}</div>
      {data &&
...
const RootItem = forwardRef<RootItemRef, RootItemProps>((props, ref) => {
  ...
  return (
    <div className="border-b border-[#E5E7EB] border-dashed">
      <div
        onClick={() => setIsCollapse((prev) => !prev)}
        className={clsx("cursor-pointer bg-white min-h-[80px] px-4 py-3 grid grid-cols-[14px_1fr] gap-3 items-center", {
          "border-b border-[#E5E7EB] border-dashed": !isCollapse,
        })}
      >
        <Image
...
const ActivityItem = (props: any) => {
  ...
  const activityLink = `/learning/${permalink}?activityType=${data.activityType}&activityId=${data.activityId}&tab=syllabus`;
    ...
    >
      <div className="grid grid-cols-[20px_1fr] gap-3 items-center">
        ...
        <div className={clsx("grid gap-3 pr-4 grid-cols-[1fr_auto]")}>
          ...
          {isPreview && (
            <div className="text-navy-primary !leading-[20px] text-right text-xs">{t("Learning_Preview")}</div>
          )}
          {isEnrolled && (
            <Checkbox
              classNames={{
