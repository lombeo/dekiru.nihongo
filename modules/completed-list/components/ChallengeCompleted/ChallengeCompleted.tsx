...
import { Group, Loader, Pagination, Text, TextInput } from "@mantine/core";
...
export default function ChallengeCompleted(props: any) {
  const { id } = props;
  ...
  const fetch = async () => {
    setIsLoading(true);
    const res = await CodingService.getUserCompletedChallenge({
            ...
            data?.results.length > 0 ? (
              <div className="mt-5 grid lg:grid-cols-4 grid-cols-1 gap-[20px]">
                {data.results.map((item) => {
                  return <ChallengeItem data={item} key={item.challengeId} />;
                })}
              </div>
            ) : (
