...
export default function PaymentManagement() {
  ...
  const [courseRevenue, setCourseRevenue] = useState<{ name: string; revenue: number }[]>([]);
  ...
  const fetchRequests = async () => {
    ...
    if (result.success) {
      return result.data.map((item: any) => ({
        ...
        user: item.user,
        course: item.course,
        status: item.status === 0 ? "accepted" : item.status === 1 ? "rejected" : "pending",
        image: item.image
      }));
    } else {
  ...
  const { data: requestsData, error: requestsError, isLoading: requestsLoading } = useQuery({
    queryKey: ['requests', user],
    queryFn: fetchRequests
  });

  const { data: courseRevenueData, error: revenueError, isLoading: revenueLoading } = useQuery({
  ...
  useEffect(() => {
    if (courseRevenueData) {
      setCourseRevenue(courseRevenueData as { name: string; revenue: number }[]);
    }
  }, [courseRevenueData]);

  if (requestsLoading || revenueLoading) return <Loader />;
  if (requestsError) return <div>Error fetching requests: {requestsError.message}</div>;
  if (revenueError) return <div>Error fetching course revenue: {revenueError.message}</div>;
  ...
  return (
    ...
    <div className="container mx-auto p-4">
      <h1 className="text-4xl font-bold bg-gradient-to-r from-pink-200 to-pink-700 bg-clip-text text-transparent mb-6 animate-gradient">Quản Lý Thanh Toán</h1>
        ...
        <Card className="bg-gradient-to-r from-purple-400 to-pink-500 text-white">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <p className="text-5xl font-bold">{(courseRevenue.reduce((sum, course) => sum + course.revenue, 0)).toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</p>
            </div>
          </CardContent>
        </Card>
                        ...
                        <TableRow key={req.id}>
                          ...
                          <TableCell>
                            <Badge variant={
                              ...
                              req.status === "accepted" ? "secondary" : "destructive"
                            }>
                              {req.status}
                            </Badge>
                          </TableCell>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                ...
                                <TooltipContent side="right">
                                  <Image src={req.image} alt={`Hình ảnh của ${req.user}`} className="max-w-sm rounded-lg shadow-lg" width={600} height={600} />
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </TableCell>
