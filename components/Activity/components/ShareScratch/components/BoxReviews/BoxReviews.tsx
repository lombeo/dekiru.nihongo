...
import { Button, Collapse, Progress, TextInput, Textarea, clsx } from "@mantine/core";
...
const BoxReviews = (props: BoxReviewsProps) => {
  ...
  return (
    <div id="reviews" className="border-l shadow-[0px_5px_12px_0px_#0000001A] p-4 flex flex-col gap-5">
      <div>
        <div className="flex items-center gap-2">
          <div className="text-[24px] font-semibold mb-2">{(data?.averageRating || 0)?.toFixed(1)}</div>
          <div className="text-[#8899A8] font-semibold text-sm">
            ({FunctionBase.formatNumber(data?.rowCount || 0)} {t(data?.rowCount > 1 ? "reviews" : "review")})
          </div>
        </div>
        <StarRatings size="md" rating={data?.averageRating || 0} />
      </div>
...
export const FromRating = (props: FromRatingProps) => {
  const { contextId, contextType, onSuccess } = props;
  ...
  const submitEnquiryForm = async (data, gReCaptchaToken) => {
    ...
    if (res?.data?.success) {
      Notify.success(t("Rating successfully!"));
      reset(initialValues);
      onSuccess();
    } else {
      if (res?.data?.message) {
      ...
      authorized
        ...
        : {
            ...
            userName: yup
              ...
              .max(512, t("Your comment must be less than 512 characters")),
            email: yup.string().required(t("Email is required")).email(t("Invalid email")).trim(t("Email is required")),
          }
    );
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-5 justify-between items-center">
        ...
        />
        <Button size="sm" color="dark" onClick={handleClickSubmit}>
          {t("Submit a review")}
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="border-b flex flex-col gap-3 border-[#E5E7EB] pb-8">
      <div className="grid grid-cols-[auto_1fr] gap-3 items-center">
        ...
        <div className="flex flex-col flex-none mt-1 font-semibold">
          {data?.user?.userId ? (
